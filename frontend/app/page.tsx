...
export default async function Home() {
  ...
  return (
    <div className="flex flex-col gap-16 pb-16">
      {/* Hero Section */}
      <section className="relative rounded-3xl overflow-hidden bg-secondary mt-4 transition-all duration-300">
        <div className="absolute inset-0 z-0">
          ...
          <div className="absolute bottom-0 left-0 w-80 h-80 bg-[#FF7A00]/5 rounded-full blur-3xl translate-y-1/3 -translate-x-1/3"></div>
        </div>
        
        <div className="relative z-10 px-6 py-16 md:py-24 lg:px-16 flex flex-col md:flex-row items-center gap-12">
          <div className="flex-1 space-y-8 text-center md:text-left">
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/50 dark:bg-white/10 backdrop-blur-sm border border-white/20 text-sm font-semibold text-[#FF7A00]">
            ...
            <p className="text-lg text-text-muted max-w-xl mx-auto md:mx-0 font-medium">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center md:justify-start">
              ...
              <Link href="/products?trending=true">
                <Button variant="outline" size="lg" className="w-full sm:w-auto bg-white/50 dark:bg-transparent backdrop-blur-sm border-secondary-dark/30 hover:bg-[#FF7A00]/10">
                  View Trending
                </Button>
              </Link>
            </div>
      ...
      <section>
        <div className="flex items-center justify-between mb-8">
          <h2 className="text-2xl md:text-3xl font-extrabold text-text-main tracking-tight">Shop by Category</h2>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-7 gap-4">
          {mockCategories.map((category) => (
            <Link 
              ...
              className="flex flex-col items-center justify-center p-6 rounded-2xl bg-surface border border-secondary-dark/20 shadow-soft hover:shadow-hover hover:border-[#FF7A00]/30 transition-all duration-300 group"
            >
              <div className="w-16 h-16 rounded-full bg-secondary flex items-center justify-center mb-4 group-hover:scale-110 transition-transform duration-300 group-hover:bg-[#FF7A00] group-hover:text-white">
                <span className="font-bold text-xl">{category.charAt(0)}</span>
              </div>
              <span className="text-sm font-bold text-center text-text-muted group-hover:text-[#FF7A00] transition-colors">
                {category}
              </span>
            </Link>
          ))}
      ...
      <section>
        <div className="flex items-center justify-between mb-8">
          <h2 className="text-2xl md:text-3xl font-extrabold text-text-main tracking-tight">Featured Products</h2>
          <Link href="/products" className="text-[#FF7A00] font-bold flex items-center gap-1 hover:underline">
      ...
      {/* Features Banner */}
      <section className="bg-[#1C1917] dark:bg-[#1C1917] rounded-3xl p-8 md:p-12 text-white border border-secondary-dark/10">
      ...
      <section>
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-2xl md:text-3xl font-extrabold text-text-main tracking-tight">Trending Now</h2>
          </div>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
